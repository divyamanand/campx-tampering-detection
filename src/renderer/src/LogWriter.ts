...
class LogWriter {
  ...
  static async selectLogsDirectory(): Promise<FileSystemDirectoryHandle> {
    try {
      const dirHandle = await window.showDirectoryPicker({
        mode: "readwrite",
        startIn: "desktop" as FileSystemStartingPoint,
      });
      return dirHandle;
    } catch (error) {
      ...
      throw new Error(`Failed to select logs directory: ${err.message}`);
    }
  }
   ...
   */
  static async readLogsFile(dirHandle: FileSystemDirectoryHandle): Promise<Record<string, TransformedLogs>> {
    try {
      ...
      throw new Error(`Failed to read logs.json: ${err.message}`);
    }
  }
  ...
  static async writeLogsFile(
    ...
    try {
      ...
      throw new Error(`Failed to write logs.json: ${(error as Error).message}`);
    }
  }
  ...
  ): Promise<Record<string, TransformedLogs>> {
    try {
  ...
  static async verifyAndGetRetryPages(dirHandle: FileSystemDirectoryHandle): Promise<VerificationResult> {
    try {
      ...
      const bestCounts: BestCounts = {};

      // First pass: Find the maximum count for each page number
      Object.entries(logs).forEach(([, fileLog]) => {
      ...
      Object.entries(logs).forEach(([fileName, fileLog]) => {
        Object.entries(fileLog).forEach(([pageNumber, pageData]) => {
          ...
          const needsRetry = qrCount < bestCounts[pageNumber].QRCode || code128Count < bestCounts[pageNumber].Code128;
      ...
      throw new Error(`Failed to verify and get retry pages: ${(error as Error).message}`);
    }
  }
}
